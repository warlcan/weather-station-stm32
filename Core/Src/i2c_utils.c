...
#define I2C_TIMEOUT_FLAG_MS 5

#define I2C_RECEIVE_TRANS_REG_SIZE 1

extern volatile uint32_t system_ticks;

bool I2C_TransmitData(I2C_TypeDef *I2Cx, uint8_t slave_addr, uint8_t *cmd_data, uint8_t cmd_size) {
    if (!WAIT_FLAG(!LL_I2C_IsActiveFlag_BUSY(I2Cx), I2C_TIMEOUT_FLAG_MS)) {
    ...
    for (uint8_t i = 0; i < cmd_size; i++) {
        if(!WAIT_FLAG(LL_I2C_IsActiveFlag_TXIS(I2Cx), I2C_TIMEOUT_FLAG_MS)) {
            BSP_ErrorSet(ERR_I2C_TXRX);
            return false;
        }
        LL_I2C_TransmitData8(I2Cx, cmd_data[i]);
    }
    if (!WAIT_FLAG(LL_I2C_IsActiveFlag_STOP(I2Cx), I2C_TIMEOUT_FLAG_MS)) {
...
bool I2C_ReceiveData(I2C_TypeDef *I2Cx, uint8_t slave_addr, 
    ...
    for (uint8_t i = 0; i < data_size; i++) {
        if(!WAIT_FLAG(LL_I2C_IsActiveFlag_RXNE(I2Cx), I2C_TIMEOUT_FLAG_MS)) {
            BSP_ErrorSet(ERR_I2C_TXRX);
            return false;
        }
...
bool I2C_ReceiveRegsData(I2C_TypeDef *I2Cx, uint8_t slave_addr, 
    ...
    LL_I2C_HandleTransfer(I2Cx, slave_addr, 
                        LL_I2C_ADDRSLAVE_7BIT, I2C_RECEIVE_TRANS_REG_SIZE, 
    ...
    if (!WAIT_FLAG(LL_I2C_IsActiveFlag_TC(I2Cx), I2C_TIMEOUT_FLAG_MS)) {
        BSP_ErrorSet(ERR_I2C_STOP);
        return false;
    }
